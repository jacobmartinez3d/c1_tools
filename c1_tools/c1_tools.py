        ...
        if len(writeNodes) > 0:
            ...
            for i in range(len(writeNodes)):
                ...
                if flag == True:
                    ...
                    n.setXYpos(connectNodes[i].xpos(), connectNodes[
                               i].ypos() - (connectNodes[i].height() / 100) * 5)
...
def titles_qc():
    def validateFrames(type):
        ...
        for thing in sorted(os.listdir(os.path.join(titlesFolder_dct[folder], type))):
            ...
            if validated_frame:
                titleName = os.path.basename(
                    titlesFolder_dct[folder]) + "_" + validated_frame.group(1)
                print("'" + validated_frame.string +
    ...
    def writeSummary(type, data):
        ...
        #   -need to handle missing folders and misnamed frames before continuing, possibly giving
        # user option to skip those folders.
        # ------------------------------------------------------------------------------------------
        result = ""
    ...
    # PRECHECKS
    re_framePattern = re.compile(
        titleName + "_(?i)(left|l|right|r|mono|m)\.(\d{1,})\.(fbx|png|tiff?|dpx)")
    # group 1: left|right|mono
    # group 2: frame number
    # group 3: file ext
    re_folderPattern = re.compile('(^\d{1,})_')
    # group 1: folder number
    titles_directory = os.path.abspath(nuke.getFilename(
        'Navigate to any directory with properly-named frames...'))
    titlesFolder_dct = {}
    # validate 'Titles' folder...
    for thing in os.listdir(titles_directory):
        validated_folder = re_folderPattern.match(thing)
        if validated_folder:
            titlesFolder_dct[int(validated_folder.group(1))] = os.path.join(
                titles_directory, validated_folder.string)
        ...
        for folder in titlesFolder_dct:
            try:
                ...
                if os.path.isdir(os.path.join(titlesFolder_dct[folder], 'left')):
                    ...
                    summary += ('\n' + os.path.basename(
                        titlesFolder_dct[folder]) + " left: MISSING")
                if os.path.isdir(os.path.join(titlesFolder_dct[folder], 'right')):
                    results = validateFrames('right')
                    if results:
                        missingFrames.append(results)
                    summary += ('\n' + os.path.basename(
                        titlesFolder_dct[folder]) + " right: FOUND.")
                else:
                    summary += ('\n' + os.path.basename(
                        titlesFolder_dct[folder]) + " right: MISSING")
            except:
                raise
                nuke.message(os.path.basename(titlesFolder_dct[
                             folder]) + " is missing 'left' and 'right', or 'mono' subdirectories!")

        print(missingFrames)
        concatList = open(os.path.join(
            titles_directory, 'concatList.txt'), 'w')
...
def ffmpegRender(titles=False):
    try:
        filepath = os.path.dirname(
            nuke.root().name()) + os.sep + 'Prerenders' + os.sep
        prerenders = filepath
        arr = os.listdir(prerenders)
    except:
        prerenders = os.path.abspath(nuke.getFilename(
            'Navigate to any directory with properly-named frames...'))
        arr = os.listdir(prerenders)
    ...
    for thing in arr:
        ...
        if match.group(6) and match.group(4):
            shotName = match.group(1) + '_' + \
                match.group(2) + '_v' + match.group(3)
            # check if left/right found
            if match.group(4) == 'left':
                inputStream_left = os.path.join(
                    prerenders, shotName + '_' + match.group(4) + '.%%04d.png')
            elif match.group(4) == 'right':
                inputStream_right = os.path.join(
                    prerenders, shotName + '_' + match.group(4) + '.%%04d.png')
            # no 'left'/'right' found so it must be 'main' or 'mono'
            else:
                inputStream_main = os.path.join(
                    prerenders, shotName + '_' + match.group(4) + '.%%04d.png')
    if inputStream_left and inputStream_right:
        ...
        text = [
            ...
            inputStream_right,
            '''
                -filter_complex "[0:v]scale=iw:ih/2[top]; [1:v]scale=iw:ih/2[bottom]; [top][bottom]vstack[v]"
                -map "[v]"
                -c:v libx264
                -b:v 20m
                -minrate 20m
...
def createShotFolder(flag=None, shotFolder=None):
    ...
    if flag is not 'auto':
        # run a quick validation on the parent folder to see if user
        # mis-clicked
    ...
    data = {'found': str(results['foundDirs']), 'missing': results[
        'missing'], 'versions': retrieveShotFolderVersions(results['foundDirs'])}
    print 'Found:' + '\n', data['found']
...
def versionUp(file):
    ...
    if os.path.exists(newDir):
        nuke.message(
            "Looks like there's already a folder for version " + newVersion + ', aborting!')
        return
    os.mkdir(newDir)
    ...
    # save nuke script
    nuke.scriptSaveAs(os.path.join(
        newDir, filename[0] + '_v' + newVersion + '.nk'))
...
def retrieveLatestVersion(directory, showCode):
    ...
    try:
        ...
        print('c1_tools.retrieveLatestVersion(): Error processing something in' +
              str(directory) + '.')
...
def submitShot(filepath):
        ...
        def __init__(self, filepath, gladiator):
            ...
            def validate():
                ...
                try:
                    ...
                    exceptions.append(
                        '--> Problem retrieving Show directories. No show found for:\n' + str(fragment2[0]) + '\n\n')
                # shotFolder
                try:
                    ...
                    exceptions.append(
                        '--> Problem retrieving Shot directories. No shot found for:\n\'' + str(nukeScriptName) + '\'\n\n')
                # versionFolder
                try:
                ...
                try:
                    ...
                    exceptions.append(
                        '--> The folder you\'re working out of doesn\'t have a valid version-number.\n\n')
                # exceptions
                if len(exceptions) > 0:
                    msg = ''
                    i = 1
                ...
                for folder in os.listdir(self.gladiator):
                    ...
                    if os.path.isdir(os.path.join(self.gladiator, folder)):
                        ...
                        if folder.split('_')[-1] == showCode:
                            serverShowFolder = os.path.join(
                                os.path.join(self.gladiator, folder), '6.VFX')
                ...
                for folder in os.listdir(serverShowFolder):
                    ...
                    if os.path.isdir(os.path.join(serverShowFolder, folder)):
                        ...
                        if folder.split(showCode + '_')[-1] == shotName:
                            serverShotFolder = os.path.join(
                                serverShowFolder, folder)
    ...
    if not data.validated:
        ...
        data.dialogueText = nuke.Text_Knob('', '', 'Latest version of this shot on Gladiator is: ' + data.shotName + '_v' + str(
            data.versionFolder.ver.remote).zfill(3) + '.\n\nYour file\'s version already exists! Continue submission as:')
        ...
        if data.versionFolder.ver.remote == 0:
            data.dialogueText = nuke.Text_Knob(
                '', '', 'This shot has no prior submissions.\n\nContinue as first submission:')
        else:
            data.dialogueText = nuke.Text_Knob('', '', 'Latest version of this shot on Gladiator is: ' + data.shotName + '_v' + str(
